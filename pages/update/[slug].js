...
export default function update({
  ...
  const [selectedIcon, setSelectedIcon] = useState(documentation.icon);
  ...
  const [iconDisplayed, setIcondisplayed] = useState(null);

  const [formData, setFormData] = useState(documentation);

  const [originalFormData, setOriginalFormData] = useState(documentation);
  ...
  useEffect(() => {
    ...
    setFormData((prevData) => ({ ...prevData, icon: selectedIcon }));
  }, [selectedIcon]);

  useEffect(() => {
    if (JSON.stringify(documentation) !== JSON.stringify(formData)) {
      ...
      try {
        if (hasFormChanges) {
          const response = await axios.put(
            server + `/api/documentations/${formData.id}`,
            formData,
            {
              headers: {
          ...
          setIsUpdatingDocs(false);
          router.push(`/documentation/${formData.slug}`);
        }
